			...
			function (data) {
				...
				$('#placeOrder').click(function() {
					if(!$(this).hasClass('greyedOut')) {
						...
						console.log("Why can't I place an order?")
						userAlert("Be sure to complete the Fullfillment method and enter a valid email address.");
					}
				})
			});
		});
					...
					if(k.indexOf(id)!=-1) {
						for(os=0;os<products[j].options.length;os++) {
							...
							for(o=0;o<products[j].options[os].options.length;o++) {
								...
								if(io==k&&cart[k]>0) {
									p=products[j];
									list=list+'<div id="'+io+'" class="listing" data="'+j+'"><div class="listingLeft"><div style="background-image:url('+p.images[0]+')"></div></div><div class="listingMid"><h3>'+p.name+' - '+products[j].options[os].options[o].name+'</h3></div><div class="listingRight" data-source="'+(cart[k]*p.price)+'"><span>$'+(cart[k]*p.price)+'</span><div id="'+io+'" class="buy" data-source="'+p.options[os].option.name+'"><button class="addToCart">Add To Cart</button><div><button class="down">&#x25BC;</button><input type="text" value="'+cart[k]+'" min=0 max='+p.options[os].option.qty+'><button>&#x25B2;</button></div></div></div></div>';
								}
							}
						}
					}
